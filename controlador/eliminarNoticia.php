...
require("conexion.php");
    ...
    if (!empty($_GET['id'])) {
        ...
        $conexion = conectar();
        if ($conexion) {
            $respuesta = mysqli_query($conexion, 'SELECT img_noticia FROM noticias WHERE id_noticia = \'' . $id . '\'');
            $respuestaDelate = mysqli_query($conexion, 'DELETE FROM noticias WHERE id_noticia = \'' . $id . '\'');
            if ($respuestaDelate) {
